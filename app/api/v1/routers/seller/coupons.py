...
async def list_seller_coupons(
    ...
    coupons = await uow.coupons.get_by_seller(current_seller.id, skip, limit)
    return coupons

@router.get("/{coupon_id}", response_model=CouponResponse)
async def get_seller_coupon(
    coupon_id: UUID,
    current_seller: Seller = Depends(get_current_seller),
    uow: UnitOfWork = Depends(get_uow)
):
    coupon = await uow.coupons.get(coupon_id)
    if not coupon or coupon.seller_id != current_seller.id:
